...
def cams_to_affinity_label(cam_label, attn_h, attn_w, mask=None, ignore_index=4, bg_index=0, ignore_bg_pairs=True):
    """use label to create affinity supervision
    cam_label: the label should be integer based
    attn_h, attn_w: spatial resolution of attention affinity prediction
    mask: spatial mask to be applied, normally restricts neighborhoodness, it is hw by hw, without the batch dimension
    ignore_bg_pairs: whether disregard pairs that are both in background (should not promote similarity on these pairs since background could include everything)
    """
    b,h,w = cam_label.shape
    cam_label_resized = F.interpolate(cam_label.unsqueeze(1).type(torch.float32), size=(attn_h, attn_w), mode="nearest")
    _cam_label = cam_label_resized.reshape(b, 1, -1)
    # _cam_label now becomes b, 1, hw
    _cam_label_rep = _cam_label.repeat([1, _cam_label.size(-1), 1])
    # now becomes b, hw, hw, means each [:, i, :] has the same layout, think about each row as a flattened label
    _cam_label_rep_t = _cam_label_rep.permute(0,2,1)
    # now, each [:, :, j] has the same layout, each column has the same layout
    aff_label = (_cam_label_rep == _cam_label_rep_t).type(torch.long)
    # take care of ignore index
    aff_label[(_cam_label_rep == ignore_index) | (_cam_label_rep_t == ignore_index)] = ignore_index
    # take care of background, if both are background
    if ignore_bg_pairs:
        aff_label[(_cam_label_rep == bg_index) & (_cam_label_rep_t == bg_index)] = ignore_index
    # apply mandatory mask
    if mask is not None:
        aff_label[:, mask == 0] = ignore_index
