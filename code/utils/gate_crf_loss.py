...
class ModelLossSemsegGatedCRF(torch.nn.Module):
    ...
    def forward(
        ...
        assert width_input % width_pred == 0 and height_input % height_pred == 0 and \
            ...
            # the sample and pred logits should share the same h/w scale and integer multiplicative factor
        ...
        # (Pdb) pp kernels.shape
        # torch.Size([12, 1, 11, 11, 256, 256])
        ...
        if compatibility is None:
            ...
            # <mtian> by derivation this is correct
        else:
    ...
    def _downsample(img, modality, height_dst, width_dst, custom_modality_downsamplers):
        # <mtian> with rgb, sure we could interpolate, BUT with semantic features, we could try upsample/downsample seglogits using interpolation
        ...
        for i, desc in enumerate(kernels_desc):
            ...
            for modality, sigma in desc.items():
                ...
                if modality == 'xy':
                    ...
                    feature = sample
                    # <mtian> in this case feature is RGB, it is the image itself! note that sample is N, C, H, W, same as any other sample map (C does not need to be three)
                    # we should allow sample to have lower resolution to pred logits
    ...
    def _create_kernels_from_features(features, radius):
        ...
        # <mtian> now (radius, radius) is a, all other location within the rectangular is b
        kernels = (-0.5 * kernels ** 2).sum(dim=1, keepdim=True).exp()
        kernels[:, :, radius, radius, :, :] = 0
        # <mtian> for each spatial location x, y \in H, W, the similarity of itself is zero.
        return kernels
