...
def is_valid_label(label, n_classes=4):
    # if label does not include ANY foreground, then it is not valid
...
class BaseDataSets(Dataset):
    def __init__(
        ...
        if self.split == 'train':
            ...
            for case in self.sample_list_:
                ...
                if is_valid_label(h5f["scribble"][:]) and is_valid_label(h5f["label"][:]):
    ...
    def __getitem__(self, idx):
        ...
        trm_keys = ["image", "scribble", "gt", "rw", "rrw"]
        sample = self.transform(sample, keys=[k for k in trm_keys if sample[k] is not None])
...
class RandomGenerator(object):
    ...
    def __call__(self, sample, keys):
        if random.random() > 0.5:
            sample = random_rot_flip(sample, keys)
        elif random.random() > 0.5:
            sample = random_rotate(sample, keys)
